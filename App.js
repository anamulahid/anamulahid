...
const RestaurantCard = (props) => {
  ...
  return (
    <div
      className="m-4 p-4 w-[260px] bg-gray-100 hover:bg-gray-300 rounded-md cursor-pointer"
      style={{
    ...
    >
      ...
      <h4 className="font-serif">{avgRating} stars</h4>
      <h4 className="font-serif">₹{costForTwo / 100} FOR TWO</h4>
      <h4 className="font-serif">{deliveryTime} minutes</h4>
    </div>
  );
};
...
const [filteredRestaurant,setFilteredRestaurant]=useState(resList);
  ...
  const fetchData = async ()=>{
    ...
    console.log(json)
     setListOfRestaurants (json?.data?.cards[0].data)
    ...
    <div className="body">
      <div className="search-container">
        ...
        <button className="px-4 py-2 bg-green-200 m-4 rounded-lg" onClick={()=>{
         ...
         const filteredRestaurant = listOfRestaurants.filter((res)=>res.data.name.toLocaleLowerCase().includes(searchText.toLocaleLowerCase())  );
